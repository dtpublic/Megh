...
public class DimensionsQueryExecutor implements QueryExecutor<DataQueryDimensional, QueryMeta, MutableLong, Result> {
  ...
  {
    for (int offset = 0; offset < keys.size() - (operator.getRollingCount() - 1); offset++) {
      int index = offset + (operator.getRollingCount() - 1);
      ...
      for (int rollingIndex = 0; rollingIndex < operator.getRollingCount(); rollingIndex++) {
