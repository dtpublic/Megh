...
import com.datatorrent.contrib.hdht.log.HDFSLogReader;
import com.datatorrent.contrib.hdht.log.HDFSLogWriter;
import com.datatorrent.contrib.hdht.log.LogReader;
import com.datatorrent.contrib.hdht.log.LogWriter;
...
public class HDHTWalManager implements Closeable
{
  ...
  {
    runRecovery(new RecoveryContext(writeCache, null, start, end));
  }
  ...
  public void runRecovery(RecoveryContext context) throws IOException
  {
    ...
    for (long i = context.startWalPos.fileId; i <= context.endWalPos.fileId; i++) {
      LogReader<HDHTLogEntry.HDHTWalEntry> wReader = new HDFSLogReader<HDHTLogEntry.HDHTWalEntry>(bfs, new HDHTLogEntry.HDHTLogSerializer(), bucketKey, WAL_FILE_PREFIX + i);
      wReader.seek(offset);
      offset = 0;
      int count = 0;
  ...
  public void append(HDHTLogEntry.HDHTWalEntry entry) throws IOException
  {
    ...
    if (writer == null) {
      writer = new HDFSLogWriter(bfs, new HDHTLogEntry.HDHTLogSerializer(), bucketKey, WAL_FILE_PREFIX + walFileId);
    }
  ...
  {
    Map<Slice, byte[]> writeCache;
    WalPosition startWalPos;
    WalPosition endWalPos;

    public RecoveryContext(Map<Slice, byte[]> writeCache, Comparator<Slice> cmparator, WalPosition startWalPos, WalPosition endWalPos)
    {
