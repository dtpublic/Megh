...
public abstract class AbstractExpirableOrderedBucketManager<T> extends AbstractBucketManager<T>
{
  ...
  public void endWindow(long window)
  {
    ...
    ((BucketStore.ExpirableBucketStore<T>) bucketStore).recordAndMarkFilesToDelete(expiryPoint);
  }

  @Override
  public void checkpointed(long window)
  {
    ((BucketStore.ExpirableBucketStore<T>) bucketStore).checkpointed();
    ...
    bucketSlidingTimer.schedule(new TimerTask()
    {
      ...
      {
        try {
          ((BucketStore.ExpirableBucketStore<T>) bucketStore).deleteExpiredBuckets();
        }
        catch (Exception e) {
