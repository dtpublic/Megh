...
public class DimensionalCompositeSchemaTest
{
  ...
  public void testOneCompositeSchema() throws Exception
  {
    ...
    final DimensionalConfigurationSchema dimensionConfigSchema = new DimensionalConfigurationSchema(onlyOneCompositeAggregatorSchema, AggregatorRegistry.newDefaultAggregatorRegistry());
    ...
    Map<String, AbstractTopBottomAggregator> compsiteNameToAggregator = dimensional.getAggregatorRegistry().getNameToTopBottomAggregator();
    Assert.assertTrue(compsiteNameToAggregator.size()==1 && compsiteNameToAggregator.keySet().iterator().next().equals(compositeAggregatorName));
    ...
    Assert.assertTrue(compositeAggregatorNameToID.size()==1 && compositeAggregatorNameToID.keySet().iterator().next().equals(compositeAggregatorName));
    Assert.assertTrue(compositeAggregatorNameToID.values().iterator().next() == incremantalAggregatorNameToID.size());
    
    //check output map
    List<Int2ObjectMap<FieldsDescriptor>> ddIdToAggregatorIDToOutputAggregatorDescriptor = dimensionConfigSchema.getDimensionsDescriptorIDToAggregatorIDToOutputAggregatorDescriptor();
    
    List<Map<String, FieldsDescriptor>> ddIdToCompositeAggregatorToDescriptor = dimensionConfigSchema.getDimensionsDescriptorIDToCompositeAggregatorToAggregateDescriptor();
    
    //there should have 2 ddId, 0(publisher) should only have composite aggregator; 1(publisher, location) should have aggregator SUM
    {
      ...
      Assert.assertTrue(topAggregatorToDescriptor.keySet().iterator().next() == dimensional.getAggregatorRegistry().getTopBottomAggregatorNameToID().get(compositeAggregatorName));
    }
    
    {
      ...
      Assert.assertTrue(sumAggregatorToDescriptor.keySet().iterator().next() == dimensionConfigSchema.getAggregatorRegistry().getIncrementalAggregatorNameToID().get("SUM"));
    }
  }
  ...
  public void testMultiCompositeSchema() throws Exception
  {
    ...
    final DimensionalConfigurationSchema dimensionConfigSchema = new DimensionalConfigurationSchema(multiCompositeAggregatorSchema, AggregatorRegistry.newDefaultAggregatorRegistry());
    final DimensionalSchema dimensional = new DimensionalSchema(dimensionConfigSchema);
    
    Map<String, IncrementalAggregator> incrementalNameToAggregator = dimensional.getAggregatorRegistry().getNameToIncrementalAggregator();
    Map<String, AbstractTopBottomAggregator> compsiteNameToAggregator = dimensional.getAggregatorRegistry().getNameToTopBottomAggregator();
    //check composite aggregate names
    Assert.assertTrue(compsiteNameToAggregator.size()==2 && compsiteNameToAggregator.keySet().equals(Sets.newHashSet(compositeAggregatorNames)));
    ...
    //there should have 2 ddId, 0(publisher) should only have composite aggregator; 1(publisher, location) should have aggregator SUM, COUNT
    {
      Int2ObjectMap<FieldsDescriptor> topAggregatorToDescriptor = ddIdToAggregatorIDToOutputAggregatorDescriptor.get(0);
      ...
      {
        compositeAggregatorIDs.add(dimensional.getAggregatorRegistry().getTopBottomAggregatorNameToID().get(compositeAggregatorName));
      }
    ...
    
    {
      ...
      incrementalAggregatorIDs.add(dimensionConfigSchema.getAggregatorRegistry().getIncrementalAggregatorNameToID().get("SUM"));
      incrementalAggregatorIDs.add(dimensionConfigSchema.getAggregatorRegistry().getIncrementalAggregatorNameToID().get("COUNT"));
      Assert.assertTrue(sumAggregatorToDescriptor.keySet().equals(incrementalAggregatorIDs));
      
      //check value: 
      {
        //SUM: impression and cost
        FieldsDescriptor fd = sumAggregatorToDescriptor.get(dimensionConfigSchema.getAggregatorRegistry().getIncrementalAggregatorNameToID().get("SUM"));
      ...
      {
        //COUNT: cost only
        FieldsDescriptor fd = sumAggregatorToDescriptor.get(dimensionConfigSchema.getAggregatorRegistry().getIncrementalAggregatorNameToID().get("COUNT"));
        Map<String, Type> fieldToType = fd.getFieldToType();
  ...
  public void testSchemaComposite() throws Exception
  {
    ...
    final DimensionalConfigurationSchema dimensionConfigSchema = new DimensionalConfigurationSchema(eventSchemaJSON, AggregatorRegistry.newDefaultAggregatorRegistry());
    final DimensionalSchema dimensional = new DimensionalSchema(dimensionConfigSchema);
    ...
    Map<String, FieldsDescriptor> expectedFirstCombinationAggregateNameToFD = Maps.newHashMap();
    {
      ...
      fieldToSerde.clear();
      fieldToSerde.put("impressions", SerdeMapPrimitive.INSTANCE);
      expectedFirstCombinationAggregateNameToFD.put("TOPN-SUM-10_location_publisher", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
    ...
    Map<String, FieldsDescriptor> expectedSecondCombinationAggregateNameToFD = Maps.newHashMap();
    {
      ...
      fieldToSerde.clear();
      fieldToSerde.put("impressions", SerdeMapPrimitive.INSTANCE);
      expectedSecondCombinationAggregateNameToFD.put("TOPN-SUM-10_advertiser", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
    ...
    Map<String, FieldsDescriptor> expectedThirdCombinationAggregateNameToFD = Maps.newHashMap();
    {
      ...
      expectedThirdCombinationAggregateNameToFD.put("TOPN-SUM-10_location", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
    {
      ...
      fieldToSerde.clear();
      fieldToSerde.put("cost", SerdeMapPrimitive.INSTANCE);
      expectedThirdCombinationAggregateNameToFD.put("TOPN-COUNT-10_location", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
    {
      ...
      fieldToSerde.clear();
      fieldToSerde.put("cost", SerdeMapPrimitive.INSTANCE);
      expectedThirdCombinationAggregateNameToFD.put("BOTTOMN-SUM-10_location", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
    {
      ...
      fieldToSerde.clear();
      fieldToSerde.put("cost", SerdeMapPrimitive.INSTANCE);
      expectedThirdCombinationAggregateNameToFD.put("BOTTOMN-AVG-10_location", new FieldsDescriptor(fieldToType, fieldToSerde));
    }
...
//  expectedNameToAggregator.put("BOTTOMN-SUM-10_location", new AggregatorBottom().withCount(10).withEmbedAggregator(incrementalNameToAggregator.get("SUM"))
    ...
    //expectedCommonAggregatorIds.add(incremantalAggregatorNameToID.get("COUNT"));  //BOTTOMNN-AVG-20 depends combination location
    ...
    // depend combination {"advertiser", "location"}, {"location", "publisher"}, {"location", "advertiser", "publisher"} with aggregator {"SUM", "COUNT"} 
    ...
    autoGeneratedKeysToAggregatorIds.put(Sets.newHashSet("location", "advertiser", "publisher"), Sets.newHashSet(sumId, countId));
