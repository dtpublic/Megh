...
/**
 * BaseFTPFileSystem extends from FTPFileSystem(Version: 2.7.1)
 * All API's in this class are either public or protected
 */
public class BaseFTPFileSystem extends FTPFileSystem
{
  public static final Log LOG = LogFactory
      .getLog(BaseFTPFileSystem.class);
   ...
   */
  // Changed private to protected
  protected Path makeAbsolute(Path workDir, Path path) {
    if (path.isAbsolute()) {
      return path;
    }
   ...
   */
  // Changed private to protected
  protected boolean exists(FTPClient client, Path file) throws IOException {
    try {
   ...
   */
  // Changed private to protected
  protected boolean delete(FTPClient client, Path file, boolean recursive)
      throws IOException {
    Path workDir = new Path(client.printWorkingDirectory());
    Path absolute = makeAbsolute(workDir, file);
  ...
  // Changed private to protected
  protected FsAction getFsAction(int accessGroup, FTPFile ftpFile) {
  ...
  // Changed private to protected
  protected FsPermission getPermissions(FTPFile ftpFile) {
   ...
   */
  // Changed private to protected
  protected FileStatus[]    listStatus(FTPClient client, Path file)
      throws IOException {
    Path workDir = new Path(client.printWorkingDirectory());
    Path absolute = makeAbsolute(workDir, file);
    FileStatus fileStat = getFileStatus(client, absolute);
   ...
   */
  // Changed private to protected
  protected FileStatus getFileStatus(FTPClient client, Path file)
      throws IOException {
   ...
   */
  // Changed private to protected
  protected boolean mkdirs(FTPClient client, Path file, FsPermission permission)
      throws IOException {
   ...
   */
  // Changed private to protected
  protected boolean isFile(FTPClient client, Path file) {
    try {
   ...
   */
  // Changed private to protected
  protected boolean isParentOf(Path parent, Path child) {
   ...
   */
  // Changed private to protected
  protected boolean rename(FTPClient client, Path src, Path dst)
      throws IOException {
    Path workDir = new Path(client.printWorkingDirectory());
